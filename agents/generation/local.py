...
class LocalGenerator(GenerativeModel):
    def __init__(self, model_name = 'Qwen/Qwen2.5-1.5B-Instruct', prompt_model=None, api_key = None, lora_path=None):
        ...
        Args:
            ...
            prompt_model: Prompt模型实例
            api_key (str): API密钥（如需要）
            lora_path (str, optional): LoRA适配器路径，如果提供将加载并合并LoRA参数
        """
    ...
    def _get_supported_params(self):
        ...
        try:
            ...
            log.error(f"Failed to get generate method signature: {e}")
    ...
    def _enhanced_json_parsing(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            log.error("Strategy 1 (direct parsing) failed")
        ...
        try:
            ...
            log.error(f"Strategy 2 (code block extraction) failed: {e}")
        ...
        try:
            ...
            log.error(f"Strategy 3 (pattern matching) failed: {e}")
        ...
        try:
            ...
            log.error(f"Strategy 4 (key-value extraction) failed: {e}")
